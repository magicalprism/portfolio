function restoreFiltersFromStorage() {
  ...
  if (savedFilters.selectedIndustry) {
    const industryInput = document.querySelector(
      `input[name="industry"][value="${savedFilters.selectedIndustry}"]`,
    );
    if (industryInput) {
      ...
      console.warn(
        `⚠️ Industry radio button not found: ${savedFilters.selectedIndustry}`,
      );
    }
  }
  ...
  setTimeout(() => {
    console.log(
      "🔁 Running updateDynamicVisibility() after restoring filters.",
    );
