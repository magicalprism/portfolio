function updateDynamicVisibility() {
  ...
  document.querySelectorAll(".dynamic-content").forEach((section) => {
    ...
    console.log(
      `🔎 Checking section: Industry=${sectionIndustry}, Key=${sectionKey}, Show=${showSection}, New Display=${section.style.display}`,
    );
    ...
    if (showSection) {
      ...
      if (parentContainer) {
        ...
        console.log(
          `✅ Ensuring parent container is also visible: ${parentContainer.getAttribute("data-section")}`,
        );
      }
    }
  });
