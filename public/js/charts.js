...
const chartColors = {
  doughnut: [
    styles.getPropertyValue("--doughnut-color-1").trim(),
    styles.getPropertyValue("--doughnut-color-2").trim(),
  ],
};
...
/*  function initializeChart(chartConfig) {
    ...
    if (ctx) {
      new Chart(ctx, {
        type: chartConfig.type || 'bar', // Default to 'bar' if no type specified
